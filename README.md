...
mais capacite de double coffre (54 slots), base sur un Coffre de l'End
(Material.ENDER_CHEST) qui ne fusionne jamais avec un autre par
construction du jeu - aucune protection anti-fusion n'est donc necessaire.
...
├── emeraldchest/ logique du Coffre en Emeraude (base sur Ender Chest)
...
config.yml (reglages generaux : protection, debug), custom-enchants.yml
(couts et limites des Custom Enchants), custom-items.yml (statistiques +
mapping de textures des Custom Items), gui.yml (reglages d'affichage des
interfaces), recipes.yml (recettes de craft custom, aucune pour le
moment), messages.yml (tous les messages du plugin : enchants.*, items.*).
...
belacustoms.items.use). Mettez a jour vos groupes de permissions. La
commande admin principale est renommee /belacustoms (etait
/belarionenchants). /citem est conservee telle quelle.
